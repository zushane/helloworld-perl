...
while [ "$proceed_after_repository" -eq 0 ] ; do 
    ...
    read -r -p "${cyan}Enter GitHub Container repository username${noColour} [${bold}zushane${noColour}]: " repository_name
...
docker build -t "ghcr.io/${repository_image_name}:${new_tag}" -t "ghcr.io/${repository_image_name}:latest" .

# Prompt for docker hub login. 
printf "${cyan}Clearing any existing GitHub login.${noColour}\n"
docker logout ghcr.io >/dev/null 2>&1


printf "${cyan}Please log in to Dockerhub when prompted.${noColour}\n"
while ! docker login ghcr.io ; do
    sleep 1
done

# Push 'latest' and new tags up to dockerhub.
docker push "ghcr.io/${repository_image_name}:${new_tag}" 
docker push "ghcr.io/${repository_image_name}:latest"
